...
public class NodeUtility {
    ...
    private static void computeAddressInverseMap(GOVMinimalPerfectHashFunction<CharSequence> addressMap) throws IOException {
        ...
        ProgressLogger progress = new ProgressLogger(LoggerFactory.getLogger(NodeUtility.class));
        progress.expectedUpdates = addressMap.size64();
        buildInverseMap(addressMap, addresses, addressesInverseMap, progress);
        logger.info("Done!");
    }

    private static void buildInverseMap(GOVMinimalPerfectHashFunction<CharSequence> map, Iterator<CharSequence> iterator, Path addressesInverseMap, ProgressLogger progress) throws IOException {
        Long2ObjectOpenHashMap<String> inverse = new Long2ObjectOpenHashMap<>();
        iterator.forEachRemaining(line -> {
            inverse.put(map.getLong(line), line.toString());
