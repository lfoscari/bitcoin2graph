 ...
 *
 * <p>Note that the {@link #step()} method is not available: due to the need for some synchronization logic, only {@link #stepUntil(SpectralRanking.StoppingCriterion)}
 * is available.
 *
 ...
 * you must pass to the {@linkplain #PageRankAlphaOne(ImmutableGraph, int, Logger) constructor} the <strong>transpose</strong>
 * of the graph.
 *
...
public class PageRankAlphaOne extends PageRank {
	private final static Logger LOGGER = LoggerFactory.getLogger(PageRankAlphaOne.class);
	 ...
	 */
	public PageRankAlphaOne(final ImmutableGraph transpose, final int requestedThreads, final Logger logger) {
	 ...
	 * @param transpose the transpose of the graph on which to compute PageRank.
	 */
	public PageRankAlphaOne(final ImmutableGraph transpose) {
	...
	private final class IterationThread extends Thread {
		...
		public void run() {
			try {
				...
				final ImmutableGraph graph = PageRankAlphaOne.this.graph.copy();
				final BitSet buckets = PageRankAlphaOne.this.buckets;
				final int[] outdegree = PageRankAlphaOne.this.outdegree;
				final int n = PageRankAlphaOne.this.n;
				final double alpha = PageRankAlphaOne.this.alpha;
				final DoubleList preference = PageRankAlphaOne.this.preference;
			...
			if (iteration > 0) {
				...
				if (preference != null)
					if (danglingNodeDistribution == null)
						for(int i1 = n; i1-- != 0;) rank[i1] += adjustment;
					else
						for(int i2 = n; i2-- != 0;) rank[i2] += adjustment * danglingNodeDistribution.getDouble(i2);
				else
				if (danglingNodeDistribution == null)
					for(int i3 = n; i3-- != 0;) rank[i3] += adjustment;
				else
					for(int i4 = n; i4-- != 0;) rank[i4] += adjustment * danglingNodeDistribution.getDouble(i4);

				iterationLogger.setAndDisplay(iteration);

				if (stoppingCriterion.shouldStop(PageRankAlphaOne.this)) {
					completed = true;
					return;
				}
			}
	...
	public double normDelta() {
		return Norm.L_1.compute(rank, previousRank);
	}
	...
	public static void main(final String[] arg) throws IOException, JSAPException, ClassNotFoundException, ConfigurationException {
		...
		final SimpleJSAP jsap = new SimpleJSAP(PageRankAlphaOne.class.getName(), "Computes PageRank of a graph, given its transpose, using a parallel implementation of the power-series method."
		...
		final PageRankAlphaOne pr = new PageRankAlphaOne(graph, threads, LOGGER);
