 ...
 * you must pass to the {@linkplain #PageRankParallelGaussSeidelAlphaOne(ImmutableGraph, int, Logger) constructor} the <strong>transpose</strong>
 * of the graph.
 *
...
public class PageRankParallelGaussSeidelAlphaOne extends PageRank {
	private final static Logger LOGGER = LoggerFactory.getLogger(PageRankParallelGaussSeidelAlphaOne.class);
	 ...
	 */
	public PageRankParallelGaussSeidelAlphaOne(final ImmutableGraph transpose, final int requestedThreads, final Logger logger) {
	 ...
	 * @param transpose the transpose of the graph on which to compute PageRank.
	 */
	public PageRankParallelGaussSeidelAlphaOne(final ImmutableGraph transpose) {
	 ...
	 * @param logger a logger that will be passed to <code>super()</code>.
	 */
	public PageRankParallelGaussSeidelAlphaOne(final ImmutableGraph transpose, final Logger logger) {
		this(transpose, 0, logger);
	}
	...
	private final class IterationThread extends Thread {
		...
		public void run() {
			try {
				...
				final ImmutableGraph graph = PageRankParallelGaussSeidelAlphaOne.this.graph.copy();
				final int n = PageRankParallelGaussSeidelAlphaOne.this.n;
				final double oneMinusAlpha = 1 - alpha;
				final double oneMinusAlphaOverN = oneMinusAlpha / n;
				final double[] rank = PageRankParallelGaussSeidelAlphaOne.this.rank;
				final int[] outdegree = PageRankParallelGaussSeidelAlphaOne.this.outdegree;
				final BitSet buckets = PageRankParallelGaussSeidelAlphaOne.this.buckets;
				final boolean pseudoRank = PageRankParallelGaussSeidelAlphaOne.this.pseudoRank;
				final double alpha = PageRankParallelGaussSeidelAlphaOne.this.alpha;
				final DoubleList danglingNodeDistribution = PageRankParallelGaussSeidelAlphaOne.this.danglingNodeDistribution;
				final DoubleList preference = PageRankParallelGaussSeidelAlphaOne.this.preference;
				final KahanSummation s = new KahanSummation();

				for(;;) {
					barrier.await();
					if (completed) return;
					final double danglingRank = PageRankParallelGaussSeidelAlphaOne.this.danglingRank;
					...
					for(;;) {
						...
						synchronized (PageRankParallelGaussSeidelAlphaOne.this) {
							PageRankParallelGaussSeidelAlphaOne.this.danglingRankAccumulator += danglingRankAccumulator;
							if (normVector != null) PageRankParallelGaussSeidelAlphaOne.this.normDelta = Math.max(PageRankParallelGaussSeidelAlphaOne.this.normDelta, norm);
							else PageRankParallelGaussSeidelAlphaOne.this.normDelta += norm;
						}
					}
				}
			}
		...
		barrier = new CyclicBarrier(numberOfThreads, () -> {
			if (iteration > 0) {
				...
				*/
				...
				if (stoppingCriterion.shouldStop(PageRankParallelGaussSeidelAlphaOne.this)) {
					completed = true;
					return;
				}
	...
	public static void main(final String[] arg) throws IOException, JSAPException, ClassNotFoundException, ConfigurationException {

		final SimpleJSAP jsap = new SimpleJSAP(PageRankParallelGaussSeidelAlphaOne.class.getName(), "Computes PageRank of a graph, given its transpose, using a parallel implementation of Gauss-Seidel's method."
		...
		final PageRankParallelGaussSeidelAlphaOne pr = new PageRankParallelGaussSeidelAlphaOne(graph, threads, LOGGER);
